...
mkdir -p "$DEST"
echo "==> Copying $WIC_FILENAME to $DEST ..."
cp "$PROJECT_ROOT/$WIC_FILENAME" "$DEST/$WIC_FILENAME"

echo "==> Cleaning up .wic from project root..."
rm -f "$PROJECT_ROOT/$WIC_FILENAME"

echo ""
echo "Done! Open Raspberry Pi Imager, choose 'Use custom image', and select:"
echo "  $DEST/$WIC_FILENAME"
